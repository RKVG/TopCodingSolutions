public class BinaryCode {
	...
	/*
    Decodes the string. The string is encoded by adding it's two neighbors to
    it's values. So: 1001001 becomes 1111111
    In some cases the message may decode to two different values depending on
    if pos1 = 0 or 1.
    In other cases a value of 0 or 1 may lead to a message that cannot be
    decoded.
	...
	private static String decode(char[] message, char pos1) {
        ...
        /*
        Create a return array that is 1 element larger than the message. This
        extra space will make it easier to compute the last element, and will
        ultimately get chopped off before returning anyway.
         */
		...
		for (int i = 1; i < ret.length; i++) {
            ...
            (one position back in the message) - (one position back in the
            return array) - (2 positions back in the return array)
            Write this out on papers until you're convinced that it works.
             */
			int val = a - b - c;

            /*
            If we arrive at a non-binary digit, then the encoding was
            impossible given the value of pos1.
              */
			if ((val > 1) || (val < 0)) {
				return "NONE";
			}
